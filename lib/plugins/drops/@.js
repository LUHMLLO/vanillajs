...
export default class Drops {
	...
	init(drops = this.getDrops()) {
		...
		// Observe for new dropdown elements added to the document
	...
	listenClicksOutside = (event) => {
		this.getDrops().forEach((dropdown) => {
			...
			// Only close the dropmenu if the clicked element is not inside the dropdown and the dropdown is open.
			if (
				dropmenu &&
	...
	/**
	 * Creates a new Drops instance and initializes it.
	 */
	constructor() {
		this.init();
	}
